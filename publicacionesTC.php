...
$mat = isset($_GET['mat']) ? trim($_GET['mat']) : '';
if ($mat === '') {
    $mat = 'Programacion';
}

// Obtener posts según la materia recibida por URL
$stmt = $pdo->prepare("SELECT posts.*, usuarios.usuario AS usuario_nombre, usuarios.img_perfil AS usuario_foto, usuarios.idusuario AS idusuarioP
    ...
    WHERE LOWER(posts.materia) = LOWER(:mat)
...
<body class="videojuegos-body">
    <header class="header-Relaciones">
        <p class="videojuegos-title"><?= htmlspecialchars(ucfirst(strtolower($mat))); ?></p>
    </header>

    <div class="publicaciones-container">
        <div class="publicar-contenedor">
            <div class="publicar-box">
                <a href="crearPost.php?user=<?= urlencode($_GET['user']) ?>&i=<?= urlencode($_GET['i']) ?>&mat=<?= urlencode($mat) ?>" class="publicar-link">
        ...
        function mostrarComentarios($pdo, $idPublicacion, $idPadre = null, $margen = 0, $user, $i, $mat) {
            if ($idPadre === null) {
                $stmt = $pdo->prepare("
            ...
            while ($coment = $stmt->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        <div style="flex:1">
                            ...
                            <div>
                                ...
                                <div id="respuesta-<?= $coment['id'] ?>" class="respuesta-form">
                                    <form class="respuesta-form-ajax" data-post-id="<?= $idPublicacion ?>" data-parent-id="<?= $coment['id'] ?>">
                ...
                mostrarComentarios($pdo, $idPublicacion, $coment['id'], $margen + 30, $user, $i, $mat);
            }
        }
        ?>
        ...
        <?php foreach ($posts as $post): ?>
            <div class="post-card" id="post-<?= $post['idPost']; ?>">
                ...
                <div id="comentarios-<?= $post['idPost'] ?>" class="comentarios-contenedor">
                    <h4>Comentarios:</h4>
                    <?php mostrarComentarios($pdo, $post['idPost'], null, 0, $_GET['user'], $_GET['i'], $mat); ?>

                    <!-- Formulario para comentario principal -->
                    <form class="comentario-form-ajax" data-post-id="<?= $post['idPost'] ?>">
    ...
    <script>
        ...
        document.addEventListener('submit', function(e) {
            if (e.target.classList.contains('comentario-form-ajax') || e.target.classList.contains('respuesta-form-ajax')) {
                e.preventDefault();
                const form = e.target;
                ...
                if (parentId) {
                    formData.append('id_padre', parentId);
                }

                fetch('ajax_comentar.php', {
                    method: 'POST',
                    body: formData
                })
